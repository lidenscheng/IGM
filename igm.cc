...
//keeping the LAF approach the same as Inoue 2008 paper 
    ...
    for(int i=0; i<yLAFvector.size(); i++)
	{
       sum += (yLAFvector[i] - NavgLAF)*(yLAFvector[i] - NavgLAF);
	}

	stdev = sqrt(sum/double(yLAFvector.size())); 
	cout << stdev << endl; 
	...
	{
//			pLAF = exp(-((i-NavgLAF)*(i-NavgLAF)/(2.0*stdev*stdev)))/sqrt(2.0*PI*stdev*stdev); 
			pLAF = exp(-(i-NavgLAF)*(i-NavgLAF)/(2.0*stdev*stdev)); 
...
//	int nMax= 5000, NN= 1000;
    ...
    lowNum= lower_bound(cppLAFvector.begin(), cppLAFvector.end(), ran);	//search through vector 
    numLAF= (lowNum-cppLAFvector.begin()); 

//	cout << "Number of DLAs in LoS: " << numDLA << endl; 

	for(int j=1; j<=numLAF; j++)
	{
		ran = rg_.Flat01();

		lowZ= lower_bound(yLAFvector.begin(), yLAFvector.end(), ran); //search through vector 

		k= (lowZ-yLAFvector.begin());

		zNext = (zVector[k-1]*(ran-yLAFvector[k-1]) + zVector[k]*(yLAFvector[k]-ran))/(yLAFvector[k]-yLAFvector[k-1]); 
		redshiftsLAF.push_back(zNext); 

//	   cout << "DLA redshift is " << zNext << endl; 

	   simulateAbsorberLAF(bdopp,NHI);
...
*/
      ...
      cout << numLAF << endl;  //counting number of DLA absorbers per line of sight 

    return;
};

//void ProbabilityDistAbsorbers::simulateAbsorberLAF(double zCurrent, double& zNext, double& bdopp, double& NHI)
void ProbabilityDistAbsorbers::simulateAbsorberLAF(double& bdopp, double& NHI)
{
